...
export default function PlayPage() {
    ...
    useEffect(() => {
        ...
        try {
            ...
            // Trigger matchmaking if enough players in queue
            if (status?.totalInQueue >= 8 && !status?.queueEntry?.matchId) {
                console.log('[AUTO-MATCHMAKING] 8+ players detected, triggering matchmaking...');
                fetch('/api/test/trigger-matchmaking', { method: 'POST' })
                    .then(r => r.json())
                    .then(data => console.log('[AUTO-MATCHMAKING]', data))
                    .catch(err => console.error('[AUTO-MATCHMAKING] Error:', err));
            }
